...
(function ($) {
    $.fn.ashtml5lightbox = function (options) {
        ...
        var ELEM_TYPE = 0, ELEM_HREF =
                1, ELEM_TITLE = 2, ELEM_GROUP = 3, ELEM_WIDTH = 4, ELEM_HEIGHT = 5, ELEM_HREF_WEBM = 6, ELEM_HREF_OGG = 7,
            ELEM_THUMBNAIL = 8, ELEM_DESCRIPTION = 9, ELEM_DIV = 10, ELEM_ORIGINALWIDTH = 11, ELEM_ORIGINALHEIGHT = 12,
            ELEM_SOCIALMEDIA = 13, ELEM_WEBLINK = 14, ELEM_WEBLINKTARGET = 15, ELEM_WEBLINKTEXT = 16;
        ...
        inst.calcElemSize = function (sizeObj, keepratio) {
            ...
            if (inst.options.responsivebarheight) if (winH <= inst.options.smallscreenheight) inst.options.barheight = inst.options.barheightonsmallheight; else inst.options.barheight = inst.options.barheightoriginal;
            if (inst.options.titlestyle ==
                "bottom") h0 -= inst.options.barheight; else if (inst.options.titlestyle == "outside") h0 -= inst.$elemData.outerHeight();
        ...
        inst.resizeLightbox = function (elemW, elemH, bAnimate, onFinish) {
            ...
            inst.$loading.hide();
            inst.$watermark.hide();
            if (inst.options.nextElem <= inst.options.curElem) if (inst.options.onlastitem && window[inst.options.onlastitem] && typeof window[inst.options.onlastitem] ==
        ...
        inst.finishCallback = function () {
            if ((inst.options.transition == "slide" || inst.options.transition == "crossfade") && inst.existingElem >= 0) if (inst.options.transition == "slide") {
                ...
                inst.$prevelem.animate({opacity: 0},
                    {duration: inst.options.transitionduration});
...
(function ($) {
    $.fn.amazingslider = function (options) {
        var ELEM_ID = 0, ELEM_SRC = 1, ELEM_TITLE = 2, ELEM_DESCRIPTION = 3, ELEM_LINK = 4, ELEM_TARGET = 5,
            ...
            ELEM_VIDEOTYPE = 16, ELEM_ALT = 17, ELEM_EXTRA = 18;
        var TYPE_IMAGE = 1, TYPE_SWF = 2, TYPE_MP3 = 3, TYPE_PDF = 4, TYPE_VIDEO_FLASH = 5, TYPE_VIDEO_MP4 = 6,
            TYPE_VIDEO_OGG = 7, TYPE_VIDEO_WEBM = 8, TYPE_VIDEO_YOUTUBE = 9, TYPE_VIDEO_VIMEO =
                10, TYPE_IFRAME = 11;
        ...
        AmazingSlider.prototype = {
            initData: function (onSuccess) {
                ...
                $(".amazingslider-slides", this.container).find("li").each(function () {
                    ...
                    if (img.length > 0) {
                        ...
                        if (buttonObj.length >
                            0) if (buttonObj.parent().is("a")) button = buttonObj.parent().clone().wrapAll("<div/>").parent().html(); else button = buttonObj.clone().wrapAll("<div/>").parent().html();
                        var video = [];
            ...
            initHtml5Lightbox: function () {
                ...
                $(".amazingslider-social-btn", this.container).click(function () {
                    ...
                    if ($(this).hasClass("amazingslider-social-facebook")) window.open("https://www.facebook.com/sharer/sharer.php?u=" + encodeURIComponent(shareUrl) + "&t=" + encodeURIComponent(shareTitle), "_blank"); else if ($(this).hasClass("amazingslider-social-twitter")) window.open("https://twitter.com/share?url=" + encodeURIComponent(shareUrl) + "&text=" + encodeURIComponent(shareTitle), "_blank"); else if ($(this).hasClass("amazingslider-social-pinterest")) window.open("https://pinterest.com/pin/create/bookmarklet/?media=" +
                ...
                if (this.options.enabletouchswipe) {
                    ...
                    touchSwipeBox.sliderTouchSwipe({
                        preventWebBrowser: false, swipeLeft: function () {
                            instance.container.trigger("amazingslider.touchswipe", [instance.curElem,
                                -1]);
                ...
                if (type == TYPE_VIDEO_YOUTUBE) this.playYoutubeVideo(href, autoplay, muted, playsinline); else if (type == TYPE_VIDEO_VIMEO) this.playVimeoVideo(href, autoplay, muted, playsinline); else if (type == TYPE_VIDEO_MP4) {
                ...
                if (ASPlatforms.isFirefox() ||
                    ASPlatforms.isOpera()) if (!ASPlatforms.canPlayMP4() && !webmhref) isHTML5 = false;
                ...
                if (ASYouTubeIframeAPIReady) {
                    ...
                    params = $.extend({}, params, {
                        "html5": 1, "autoplay": 1,
                        "rel": 0, "autohide": 1, "wmode": "transparent"
                    });
                ...
                this.container.on("amazingslider.switchtext", function (event, prev, cur) {
                    ...
                    } else if (instance.textoptions.textstyle == "dynamic") if (!instance.elemArray[cur][ELEM_TITLE] && !instance.elemArray[cur][ELEM_DESCRIPTION] &&
                        !instance.elemArray[cur][ELEM_BUTTON]) $textWrapper.fadeOut(); else $textWrapper.fadeOut(function () {
                        $title.html(instance.elemArray[cur][ELEM_TITLE]);
                ...
                if (sliderRatio < imgRatio) if (this.options.scalemode == "fit") {
                    ...
                    sliderRatio) * 50; else if (this.options.scalemode == "fit") this.curImageMarginTop = (sliderRatio - imgRatio) * 50; else {
                ...
                if (this.options.navmultirows && this.options.navdirection == "horizontal" && this.options.navthumbresponsivemode == "samecolumn") {
                    ...
                    for (var i = 1; i < $bullets.length; i++) if ((i +
                        1) % this.options.navthumbcolumn == 0) $bullets.eq(i).css({"margin-right": 0}); else $bullets.eq(i).css({"margin-right": this.options.navspacing + "px"})
                }
                ...
                $shadow.css({
                    display: "block", position: "absolute", left: l + "%", top: this.options.bottomshadowimagetop +
                        "%", width: this.options.bottomshadowimagewidth + "%", height: "auto"
                });
                        ...
                        if (this.options.navshowplaypausestandalone) {
                            ...
                            switch (this.options.navshowplaypausestandaloneposition) {
                                case "topleft":
                                    ...
                                    $navPause.css({
                                        top: 0, left: 0, "margin-left": this.options.navshowplaypausestandalonemarginx +
                                            "px", "margin-top": this.options.navshowplaypausestandalonemarginy + "px"
                                    });
                                    break;
                                case "topright":
                                    $navPlay.css({
                ...
                this.options.navimagewidth = this.options.navwidth - this.options.navborder * 2;
                this.options.navimageheight = this.options.navheight - this.options.navborder * 2;
                if (this.options.navthumbstyle == "imageandtitle") this.options.navheight += this.options.navthumbtitleheight; else if (this.options.navthumbstyle == "imageandtitledescription" || this.options.navthumbstyle ==
                ...
                if (elemTextEffect && elemTextEffect.length > 0 && "textformat" in this.options && elemTextEffect in this.options["textformat"]) this.textoptions = $.ASUpdateObject(this.textoptions, this.options["textformat"][elemTextEffect]); else this.textoptions = $.ASUpdateObject(this.textoptions, this.options);
                this.sliderTimeout.stop();
                ...
                else if (!this.elemArray[this.curElem][ELEM_SRC] && this.elemArray[this.curElem][ELEM_VIDEO].length > 0) this.playVideo(this.options.autoplayvideo); else {
            ...
            showImage: function (slideDirection, transEffect) {
                ...
                $(imgLoader).on("load", function () {
                    ...
                    if (instance.options.scalemode == "flexheight") {
                        ...
                        imgRatio) if (instance.options.scalemode == "fit") {
                    ...
                    $box.amazingsliderTransition(instance.id, $imgPrev, $imgCur,
                        {
                            ...
                            instance.isAnimating = false;
                            if (!instance.isPausedOnMouseover) instance.sliderTimeout.resume(true); else instance.sliderTimeout.unsetstartandpause()
                        }, function (timeoutid) {
        ...
        this.each(function () {
            ...
            var effects =
                ["slice", "blocks", "threedtiles", "tiles", "slide", "cssslide", "elastic", "crossfade", "fadeoutfadein", "kenburns", "threedhorizontal", "threedhorizontalwithzoom", "fade", "shuffle", "threed", "threedwithzoom", "blinds", "threedflip", "threedflipwithzoom", "flip", "flipwithzoom"];
            ...
            for (var key in this.options) if (key.substring(0, 17) == "customtransition_") {
            ...
            for (var i = 0; i < image_list.length; i++) if (this.options[image_list[i]]) if (this.options[image_list[i]].substring(0, 7).toLowerCase() !=
